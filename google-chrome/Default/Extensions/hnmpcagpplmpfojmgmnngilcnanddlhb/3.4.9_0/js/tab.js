...
const onPopupUpdated = (() => {
  ...
  const popupMatch = function (
    ...
    if (fctxt.getTabHostname() !== "" && targetURL !== "about:blank") {
      ...
      if (
        popupType === "popup" &&
        sessionSwitches.evaluateZ("no-popups", fctxt.getTabHostname())
      ) {
        fctxt.filter = {
  ...
  return function (targetTabId, openerDetails) {
    ...
    const localOpenerURL =
      openerDetails.frameId !== 0 ? openerDetails.frameURL : undefined;
    ...
    if (µb.getNetFilteringSwitch(rootOpenerURL) === false) {
      return;
    }
...
µb.tabContextManager = (() => {
  ...
  const PopupCandidate = class {
    constructor(createDetails, openerDetails) {
      ...
      this.opener = {
        ...
        popunder: false,
        trustedURL:
          createDetails.sourceTabId === µb.maybeGoodPopup.tabId
    ...
    launchSelfDestruction() {
      ...
      this.selfDestructionTimer = vAPI.setTimeout(() => this.destroy(), 10000);
    }
  };

  const popupCandidateTest = async function (targetTabId) {
    for (const [tabId, candidate] of popupCandidates) {
      if (targetTabId !== tabId && targetTabId !== candidate.opener.tabId) {
        continue;
      }
  ...
  TabContext.prototype.update = function () {
    ...
    this.rootDomain =
      domainFromHostname(this.rootHostname) || this.rootHostname;
  };
...
µb.updateToolbarIcon = (() => {
  ...
  const updateBadge = (tabId) => {
    ...
    // ws-update: dont set the extension icon
  ...
  return function (tabId, newParts = 0b0111) {
    ...
    if (currentParts === undefined) {
      self.requestIdleCallback(() => updateBadge(tabId), { timeout: 701 });
    } else {
...

{
  ...
  const checkTab = async (tabId) => {
    const tab = await vAPI.tabs.get(tabId);
    if (tab instanceof Object && tab.discarded !== true) {
      return;
    }
    µb.unbindTabFromPageStore(tabId);
  };
