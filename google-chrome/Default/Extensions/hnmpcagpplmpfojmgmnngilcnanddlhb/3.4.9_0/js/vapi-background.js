...
vAPI.Tabs = class {
  ...
  async create(url, details) {
    ...
    const subWrapper = async () => {
      ...
      const tab = await vAPI.tabs.update(toTabId(details.tabId), updateDetails);
  ...
  async open(details) {
    ...
    const targetURLWithoutHash =
      pos === -1 ? targetURL : targetURL.slice(0, pos);
    ...
    if (vAPI.windows instanceof Object === false) {
      return;
    }
    vAPI.windows.update(tab.windowId, { focused: true });
  }
  ...
  async reload(tabId, bypassCache = false) {
    ...
    try {
      await webext.tabs.reload(tabId, { bypassCache: bypassCache === true });
    } catch (reason) {}
  }
...
vAPI.setIcon = (() => {
  ...
  const titleTemplate =
    browser.runtime.getManifest().browser_action.default_title + " ({badge})";
  const icons = [
  ...
  (() => {
    ...
    const onLoaded = function () {
      ...
      for (const img of imgs) {
        const w = img.r.naturalWidth,
          h = img.r.naturalHeight;
        ctx.width = w;
        ctx.height = h;
  ...
  return async function (tabId, details) {
    ...
    if (browserAction.setIcon !== undefined) {
      if (parts === undefined || (parts & 0b0001) !== 0) {
        browserAction.setIcon(Object.assign({ tabId: tab.id }, icons[state]));
      }
      if ((parts & 0b0010) !== 0) {
...
vAPI.messaging = {
  ...
  setup: function (defaultHandler) {
    ...
    browser.runtime.onConnect.addListener((port) => this.onPortConnect(port));

    // https://bugzilla.mozilla.org/show_bug.cgi?id=1392067
    //   Workaround: manually remove ports matching removed tab.
    if (vAPI.webextFlavor.soup.has("firefox") && vAPI.webextFlavor.major < 61) {
  ...
  onPortMessage: function (request, port) {
    ...
    ubolog(
      `vAPI.messaging.onPortMessage > unhandled request: ${JSON.stringify(
        request.msg
      )}`,
      request
    );
...
vAPI.warSecret = (() => {
  ...
  const guard = function (details) {
    ...
    const pos = secrets.findIndex(
      (secret) => url.lastIndexOf(`?secret=${secret}`) !== -1
    );
    if (pos === -1) {
      return { cancel: true };
    }
...
if (!webext.menus) webext.menus = webext.contextMenus;
...
vAPI.contextMenu = webext.menus && {
  ...
  setEntries: function (entries, callback) {
    ...
    this._hash = hash;
    webext.menus.remove("uBlock0-blockElement");
    webext.menus.remove("uBlock0-blockElementInFrame");
    webext.menus.remove("uBlock0-blockResource");
    webext.menus.remove("uBlock0-subscribeToList");
    webext.menus.remove("uBlock0-temporarilyAllowLargeMediaElements");
...
vAPI.localStorage = {
  start: async function () {
    ...
    this.cache = webext.storage.local.get("localStorage").then((bin) => {
      this.cache =
        bin instanceof Object && bin.localStorage instanceof Object
          ? bin.localStorage
          : {};
    });
    return this.cache;
...
vAPI.cloud = (() => {
  ...
  if (webext.storage.sync instanceof Object === false) {
    return;
  }
  ...
  const MAX_ITEMS = webext.storage.sync.MAX_ITEMS || 512;
  const QUOTA_BYTES = webext.storage.sync.QUOTA_BYTES || 102400;
  const QUOTA_BYTES_PER_ITEM = webext.storage.sync.QUOTA_BYTES_PER_ITEM || 8192;

  const chunkCountPerFetch = 16; // Must be a power of 2
  const maxChunkCountPerItem =
    Math.floor(MAX_ITEMS * 0.75) & ~(chunkCountPerFetch - 1);
  ...
  const push = async function (details) {
    ...
    const encoded = encode instanceof Function ? await encode(json) : json;
    ...
    for (let i = 0; i < chunkCount; i++) {
      bin[datakey + i.toString()] = encoded.substr(
        i * maxChunkSize,
        maxChunkSize
      );
    }
  ...
  const pull = async function (details) {
    ...
    const json = decode instanceof Function ? await decode(encoded) : encoded;
    let entry = null;
    try {
