...
(function onSyncLoaded() {
  ...
  const getAllExtensionNames = async function () {
    ...
    if (extensionNameResponse && extensionNameResponse.hasData && extensionNameResponse.data) {
      ...
      for (let inx = 0; inx < sortedData.length; inx++) {
        ...
        if (deviceInfo && deviceInfo.deviceName && deviceInfo.extensionGUID) {
          ...
          $("#sync-extension-list-div").append(
            $("<p></p>")
              .append($("<span></span>").text(deviceName))
              .addClass(classText)
  ...
  const initialize = async function () {
    ...
    if (License.isActiveLicense() && License.get()) {
      ...
      if (!License.licenseId) {
        ...
        $("#sync-tab-message").removeClass("sync-message-hidden").addClass("sync-message-good");
        $("#btnCheckStatus").show();
      } else {
        getAllExtensionNames();
        const currentExtensionName = await SyncService.getCurrentExtensionName();
  ...
  const onLicenseUpdating = function () {
    ...
    $("#sync-tab-message").removeClass("sync-message-good").addClass("sync-message-hidden");
    $(".sync-header-message")
  ...
  const getUserConfirmation = () => {
    ...
    const dialogText =
      translate("sync_confirmation_message_I") +
  ...
  const onSettingsChanged = function (name, currentValue) {
    ...
    if (
      name === "sync_settings" &&
      currentValue &&
      !$("#current-extension-name-block").is(":visible")
    ) {
      // eslint-disable-next-line no-use-before-define
      removeSyncListeners();
      window.location.reload();
